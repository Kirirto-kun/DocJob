...
- **Route guard**: `src/middleware.ts` (Edge runtime — verifies the access-token cookie via the Edge-safe `@docjob/auth/tokens` subpath only, no Prisma/argon2) redirects unauthenticated traffic to `/login`. Public paths: `/login`, `/register`, `/landing`, `/news`, `/forgot-password`, `/reset-password`, `/api/auth/*`, `/api/images/*`. Note: `/api/attachments/*` is **not** public — it requires a session. Client-side single-flight refresh-then-retry lives in `src/lib/auth-client.ts`.
