...
/**
 * Architectural guard: @docjob/core is transport-agnostic. It must never
 ...
 * every non-test source file and fails on any leak, so a future domain
 * extraction can't silently couple core to the web transport.
 */
...
describe('@docjob/core transport-agnostic boundary', () => {
  ...
  it('imports nothing from next / next-auth / react / server-only / @/', () => {
    ...
    expect(violations, `core boundary leak:\n${violations.join('\n')}`).toEqual([]);
  });
  ...
  it('calls no Next-only request primitives (revalidatePath/cookies()/headers())', () => {
    ...
    expect(violations, `core transport-primitive leak:\n${violations.join('\n')}`).toEqual([]);
  });
});
