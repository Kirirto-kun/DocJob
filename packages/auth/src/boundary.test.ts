...
/**
 * Architectural guard: @docjob/auth is transport-agnostic. It must never
 * import Next.js, next-auth, React, `server-only`, or the web app's `@/*`
 * alias, and must never call Next-only request primitives. This test walks
 * every non-test source file and fails on any leak, so future auth work
 * (JWT/refresh/login routes) can't silently couple auth to the web transport.
 */
...
describe('@docjob/auth transport-agnostic boundary', () => {
  ...
  it('imports nothing from next / next-auth / react / server-only / @/', () => {
    ...
    expect(violations, `auth boundary leak:\n${violations.join('\n')}`).toEqual([]);
  });
  ...
  it('calls no Next-only request primitives (revalidatePath/cookies()/headers())', () => {
    ...
    expect(violations, `auth transport-primitive leak:\n${violations.join('\n')}`).toEqual([]);
  });
});
