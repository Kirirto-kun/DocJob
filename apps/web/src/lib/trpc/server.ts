 ...
 * hop to `/api/trpc`. Builds the same `ApiContext` (`{ actor }`) SP-1d's
 * `createContext` builds from a `Request`, but directly from the current
 * request's session via `getActor()` (`@/lib/action-helpers.ts`, itself
 * built on `@/lib/session.ts`'s cookie-verifying `getCurrentUser()`), since a
 * Server Component has no incoming `Request` to hand `createContext`.
 *
