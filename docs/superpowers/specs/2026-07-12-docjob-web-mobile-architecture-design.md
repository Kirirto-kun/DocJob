...
- **Дата:** 2026-07-12
- **Статус:** дизайн согласован; прошёл состязательную проверку; решения заказчика внесены (v3); **готов к планам реализации SP-0 + SP-1**
...
- **История:** v1 — первичный дизайн; v2 — учтены находки состязательной проверки против реального репо (§14); **v3 — внесены решения заказчика по §12 (CaseMode остаётся, RU+KK, баннеры/поддержка/статистика остаются, avatar→profilePhotoUrl, без дампа ChatSession)**
...
**Удаляем:** модель `ChatSession` (+ relation `Case.chatSessions`); поля `User.{solvedCaseIds,unsolvedCaseIds,patientIds,medicalRecords}`; enum-значение `Role.PATIENT`; поля `Case.{solution,taskQuestions}`; поле `User.avatar` (оставляем `profilePhotoUrl` — §12.7).
**Оставляем `CaseMode`** как чистую категоризацию (питает админ-фильтр «тип» — §12.1) — снимаем только его чат/solution-семантику (`expectedSolutionKind`, дискриминатор solution-kind). **Деструктивной enum-миграции для `CaseMode` НЕ требуется.**
...
**Миграция enum (важно):** в Postgres нет `ALTER TYPE … DROP VALUE`. Удаление значения `PATIENT` из `Role` = ручная миграция: пред-мигрировать строки с этим значением → снять `DEFAULT` → создать новый enum → `ALTER COLUMN … TYPE` → удалить старый тип. Prisma-diff это не сгенерит и упадёт на единственной оставшейся строке. (`CaseMode` остаётся — его не трогаем.)
**Архив данных: не дампим** (§12.2). Прошлая миграция `20260607000000` уже обнулила `solution`/`taskQuestions`; `ChatSession` (`messages/finalAnswer/evaluation`) удаляем без архива — по решению заказчика.
...
| **SP-1** ★ | Бэкенд-ядро: `core` из actions, `api` (tRPC), `auth` (JWT+refresh+CSRF+refresh-транспорт+oracle-fix — §5c целиком), миграция модели (§5b: PATIENT enum-рецепт, drop чат/solution/task/avatar; **CaseMode остаётся**) | SP-0 |
...
## 12. Решения заказчика (закрыто)

> **Граница удаления (директива заказчика):** удаляем **ТОЛЬКО** чат-субсистему, `taskQuestions` и ранее названное (`solution`, роль `PATIENT`, `patientIds`, `medicalRecords`, `manage-patients`). **Всё остальное остаётся как сейчас на сайте** — баннеры, поддержка, объявления, новости, заявки, рецензии, сохранённые, SEO и т.д.

1. **`CaseMode`** — **оставляем** как категоризацию; админ-каталог сохраняет фильтр «тип». Enum не трогаем, деструктивной миграции нет.
2. **Дамп `ChatSession`** — **нет**, удаляем без архива.
3. **Языки** — **RU + KK** (как сейчас на сайте). Уточнение: KK/RU — это язык **интерфейса** (i18n-каталоги); **контент кейсов — на русском**, поэтому FTS-конфиг поиска = `russian` (§6). i18n-cleanup удаляет только мёртвые ключи (чат/solution/patient), двуязычие сохраняем.
4. **Баннеры** — **оставляем** как сейчас (см. границу удаления выше). Домен `banners` + tRPC-роутер + рендер на mobile; публичное чтение манифеста сохраняем.
5. **Поддержка** — **серверное письмо** (как `sendContactMessage`), через tRPC; вложения — через media-абстракцию.
6. **Reset на mobile** — **web-based** (письмо ведёт на web-страницу reset, открывается в браузере — работает и для mobile-юзеров); deep-link в приложение — опционально на будущее. Главное — reset работает.
7. **Фото профиля** — оставляем **`profilePhotoUrl`**, `User.avatar` удаляем.
8. **Профильная карточка «Статистика»** (следствие удаления чата): счётчики solved/unsolved питались solve-механикой чата → **карточку сохраняем, но перепитываем** на актуальные данные (кол-во сохранённых кейсов; для рецензента — кол-во написанных рецензий). Так видимая фича на сайте остаётся.
...
> **Граница (см. §12):** удаляем ТОЛЬКО чат, `taskQuestions`, `solution`, `PATIENT`+patient-поля, `avatar`. Баннеры/поддержка/объявления/новости/заявки/рецензии/SEO — **остаются**. `CaseMode` — **остаётся** (категоризация).
...
Root-дашборд ветка `role==='patient'` + счётчики пациентов; `use-user-store` `UserRole 'patient'` + каст в `'PATIENT'`; `createUser/updateUser` zod (принимает `PATIENT`+`medicalRecords/patientIds`); `SerializedUser/getUsers` поля; `add-doctor` payload `patientIds`; **карточка «Статистика» профиля — перепитать (сохранить карточку; данные: сохранённые кейсы / написанные рецензии — §12.8)**; `ScenarioControls` `handleFileUpload`+ветка пациента (пишет `medicalRecords`); устаревшая ссылка `/add-patient` в `robots.ts`.
*(Легаси Genkit-флоу + `src/ai/genkit.ts`/`dev.ts` + `GOOGLE_API_KEY` — мёртвый код, НЕ видимая фича; вне границы удаления §12 → подтвердить отдельно перед удалением, по умолчанию не трогаем.)*
...
### Env
`NEXTAUTH_SECRET`→`AUTH_SECRET` (keyed-ротация); `NEXTAUTH_URL`→`AUTH_URL/APP_URL` (используется в reset-письмах); сохранить `OPENAI_API_KEY/OPENAI_MODEL` (поиск+импорт), `RESEND_API_KEY/EMAIL_FROM`, `GOOGLE_SITE_VERIFICATION/YANDEX_VERIFICATION/SITE_URL`, storage-переменные. `GOOGLE_API_KEY` — только если удаляем легаси-Genkit (§13, по умолчанию не трогаем).
