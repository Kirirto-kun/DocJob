...
- Produces: `ApiContext` gains `passwordResetBase: string` AND `contactInboxEmail: string` (see "T2 follow-up" below — this task consolidates BOTH config-injection fields in one fan-out).

**T2 follow-up folded in here (config injection):** the T2 review flagged that `core.contact.sendContactMessage` hardcodes a `CONTACT_INBOX_EMAIL` constant duplicating `apps/web/src/lib/site.ts`'s `SITE_EMAIL` (silent-drift risk; inconsistent with the injected-`EmailSender` pattern). Since this task already re-touches `ApiContext` + every context-construction site to add `passwordResetBase`, add `contactInboxEmail: string` in the SAME pass: (a) change `core.contact.sendContactMessage(input, { email })` → `sendContactMessage(input, { email, inboxEmail })` and use `inboxEmail` as the recipient (delete the core `CONTACT_INBOX_EMAIL` constant); (b) `contact` router calls `core.contact.sendContactMessage(input, { email: ctx.email, inboxEmail: ctx.contactInboxEmail })`; (c) both production context sites inject `contactInboxEmail: SITE_EMAIL` (web mount + server caller, reading `@/lib/site`), test literals inject a fixture like `'inbox@test'`. Update the contact core/api tests to pass `inboxEmail`. This removes the duplication + drift risk for good.
